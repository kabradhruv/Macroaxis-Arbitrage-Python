...
def verify_triangular_opportunity(sequence, starting_amount=100 , PROFIT_THRESHOLD_VEIRFY = 0.8):
    """
    ...
    try:
        ...
        if final_usdt is not None and arbitrage_ratio > PROFIT_THRESHOLD_VEIRFY:
            print("\n------ Arbitrage opportunity confirmed! -----------")
            print(f"------ for {sequence} and ratio - {arbitrage_ratio} -----------\n")
            file_exists = os.path.isfile("arbitrage_log.csv")
            with open("arbitrage_log.csv", mode="a", newline="") as file:
                writer = csv.DictWriter(file, fieldnames=["Sequence", "opportunity"])
                if not file_exists:
                    writer.writeheader()  # Write headers if file is new
                writer.writerow({"Sequence": sequence, "opportunity": str(arbitrage_ratio)})
        else:
            print(f"Negative result after Verificaton for - " , sequence , f" and ration - {arbitrage_ratio} ")
...
if __name__ == "__main__":
    ...
    if final_usdt is not None:
        if arb_ratio > 1:
            print(f"Arbitrage opportunity confirmed! - {arb_ratio}")
        else:
            print(f"No arbitrage opportunity found on Binance. After verifying the percent is - {arb_ratio}")
